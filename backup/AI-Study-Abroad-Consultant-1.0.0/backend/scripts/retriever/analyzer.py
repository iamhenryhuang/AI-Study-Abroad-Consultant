...
from retriever.alternative_recommendation import search_alternative, _EXP_KEY_TO_SCHOOL_ID
...
def _load_admission_experience() -> dict:
    """載入 1point3 論壇申請經驗，以 exp_key 為 key。"""
    try:
...
def analyze_user_intent(query: str, current_profile: dict) -> dict:
    ...
    try:
        ...
        return {
            "wants_alternatives": "備案" in query,
...
def alt_searcher(query: str, profile: dict, admission_stats: dict, admission_exp: dict) -> dict:
    """
    從 admission_stats + admission_exp 組裝備案文件，
    塞進 collected_docs 供 Finalizer 使用。
    """
    print(f"\n[AltSearcher] 開始尋找備案學校，申請人背景：{profile}")

    try:
        alt_school_recs = search_alternative(
            profile, admission_stats, admission_exp, top_k=MAX_ALT_SCHOOLS
        )
    except Exception as e:
...
def analyze_and_evaluate(query: str) -> dict | None:
    """協調入口：意圖分析 → 達成度評估 → 備案搜尋。"""
    ...
    should_find_alt = wants_alt or (
        bool(user_profile)
        and bool(user_aim_school)
        and not evaluate_attainability(user_profile, user_aim_school, admission_stat)
    )
