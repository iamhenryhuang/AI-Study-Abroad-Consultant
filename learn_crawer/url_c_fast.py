...
BLACKLIST_PATH_FRAGMENTS = [
    ...
    "smartrec", "my-favorites", "favorites",
    "giving", "alumni", "endowment",
    "/news/", "/events/", "/blog/",
    "hooding-ceremony", "commencement", "grad-slam", "life",
    "leave-of-absence", "withdrawal", "dissertation",
    "doctoral-committee", "thesis-committee",
    "file-your-thesis", "advancement-to-candidacy",
    "working-at", "postdoc", "for-faculty", "human-resources", "payroll",
    ...
    "civil-environmental", "dentistry","people","Faculty","life",
    "awards","staff","family","news","agentic-ai","conference","sponsorship",
    "shirt","design","competition","generating","blog","taxonomy","directory"
    ,"spotlight","spotlights","insights","profile","seminar","seminars","workshop",
    "workshops","webinar"
]
...
def normalize_url(url: str) -> str:
    try:
        ...
        return urlunparse((
            ...
            path, "", "" if STRIP_QUERY else p.query, ""
        ))
    except Exception:
        return url
...
def crawl_tree(root_url: str, max_depth: int = MAX_DEPTH) -> dict:
    ...
    print(f"\n  🌐 root: {root_url}  (depth={max_depth}, max_pages={MAX_PAGES}, workers={NUM_WORKERS})")

    for depth in range(max_depth + 1):
        if not current_layer:
            break
        if MAX_PAGES > 0 and total_crawled >= MAX_PAGES:
            print(f"  ⚠️  已達頁數上限 {MAX_PAGES}")
            break

        remaining     = MAX_PAGES - total_crawled if MAX_PAGES > 0 else len(current_layer)
        current_layer = current_layer[:remaining]

        print(f"\n  [depth={depth}] {len(current_layer)} 頁待爬")

        chunks = [current_layer[i::NUM_WORKERS] for i in range(NUM_WORKERS)]
...
def crawl_all_schools(schools: list, max_depth: int = MAX_DEPTH) -> list:
    """
    輸入：SCHOOLS 變數
    輸出：[{"school_id": "...", "urls": [...]}, ...]
...
def main():
    max_depth = int(sys.argv[1]) if len(sys.argv) > 1 else MAX_DEPTH

    print("=" * 60)
    print("CRAWLER  —  多學校多 root，輸出 classifier.py 格式")
    print("=" * 60)
    print(f"學校數量: {len(SCHOOLS)}  最大深度: {max_depth}")

    result_schools = crawl_all_schools(SCHOOLS, max_depth=max_depth)
